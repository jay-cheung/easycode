...
describe("slash commands", () => {
  test("parses prompt escape and common commands", () => {
    ...
    expect(parseSlashCommand("/file src/add.ts")).toEqual({ type: "file", action: "add", value: "src/add.ts" })
    expect(parseSlashCommand("/file clear")).toEqual({ type: "file", action: "clear" })
    ...
    expect(parseSlashCommand("/model reset")).toEqual({ type: "model" })
    expect(parseSlashCommand("/provider openai")).toEqual({ type: "provider", name: "openai" })
    expect(parseSlashCommand("/max-tokens 64000")).toEqual({ type: "maxTokens", value: 64000 })
    expect(parseSlashCommand("/max-steps 24")).toEqual({ type: "maxSteps", value: 24 })
    expect(parseSlashCommand("/max-tokens reset")).toEqual({ type: "maxTokens" })
    expect(parseSlashCommand("/max-steps default")).toEqual({ type: "maxSteps" })
    expect(parseSlashCommand("/cancel")).toEqual({ type: "cancel" })
    expect(parseSlashCommand("/stop")).toEqual({ type: "cancel" })
  ...
  test("returns error for model and provider with no args", () => {
    ...
    expect(parseSlashCommand("/file")).toEqual({ type: "error", code: "file_requires_value" })
    expect(parseSlashCommand("/max-tokens nope")).toEqual({ type: "error", code: "max_tokens_requires_value" })
    expect(parseSlashCommand("/max-steps 0")).toEqual({ type: "error", code: "max_steps_requires_value" })
  })
