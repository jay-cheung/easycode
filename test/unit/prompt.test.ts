...
describe("compact prompt", () => {
  test("puts skill-reuse rules in unified run prompts but not summary prompts", () => {
    ...
    expect(buildPrompt).toContain("During the initial planning gate, inspect only as needed with read-only tools")
    expect(buildPrompt).toContain("do not edit product files, run implementation steps, or call delegate_subagent")
    expect(buildPrompt).toContain("Key findings from bounded planning inspection")
    expect(buildPrompt).toContain("Never call delegate_subagent during the initial planning gate.")
    expect(buildPrompt).toContain("For review or analysis requests, prefer a low-risk plan")
    expect(buildPrompt).toContain("In the initial planning gate, it means bounded read-only validation or submitting the proposal plan.")
