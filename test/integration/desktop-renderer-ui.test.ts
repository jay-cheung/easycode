...
describe("desktop renderer UI integration", () => {
  ...
  test("keeps workspace menus hover-driven and tool calls grouped outside scroll-triggering state", async () => {
    ...
    expect(app).toContain('className="progress-dock"')
    ...
    expect(app).toContain("function GoalProgress")
    expect(app).toContain("function PlanProgress")
    ...
    expect(css).toContain(".goal-progress:hover .goal-popover")
    expect(css).toContain(".plan-progress:hover .plan-popover")
