  ...
  try {
    ...
    while (true) {
      ...
      if (goalState) {
        ...
        if (result.status === "completed" && shouldCompleteReadOnlyGoalPlanningResult(goalState, result.text, recentToolResults)) {
          completeGoal(result.text.trim())
          await saveSession(activeRunner.context)
          continue
        }
...
export { parseArgs, shouldCompleteReadOnlyGoalPlanningResult }
