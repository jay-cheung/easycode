...
export function buildSubagentTaskPrompt(request: SubagentTaskPacket, ledgerText: string, summary: string | undefined, priorSubagentResults: string[] = []) {
  ...
  const dynamicSections = [
    ...
    request.timeoutMs ? `Wall Clock Timeout:\nFinish or hand off within ${request.timeoutMs}ms.` : "",
    request.assignedStep ? `Assigned Plan Step:\n${request.assignedStep.stepId}: ${request.assignedStep.goal}${request.assignedStep.timeoutMs ? `\nTimeout: ${request.assignedStep.timeoutMs}ms` : ""}${request.assignedStep.doneWhen ? `\nDone When: ${request.assignedStep.doneWhen}` : ""}` : "",
